...
stop_tunnels() {
  if control_master_alive; then
    ssh -S "$CONTROL_SOCKET" -O exit "$HOST_USER@$HOST_IP" >/dev/null 2>&1 || true
  fi
  rm -f "$CONTROL_SOCKET" >/dev/null 2>&1 || true
}
...
port_is_free() {
  ...
  if command_exists netstat.exe; then
    ! netstat.exe -ano 2>/dev/null | tr -d '\r' | grep -Eq "[.:]$port[[:space:]].*LISTENING"
    return
  fi
...
start_tunnels() {
  ...
  control_master_alive || fail "SSH tunnel did not remain active. See $LOG"
...
check_ollama_models() {
  local response
  response="$(curl --fail --silent --max-time 10 "$OLLAMA_MODELS_URL" 2>/dev/null)" || return 1
  ...
  for model in \
    ...
    'qwen3-vl:30b'
  do
    printf '%s' "$response" | grep -F "$model" >/dev/null 2>&1 || return 1
  done
}
...
check_backends() {
  ...
  if curl --fail --silent --max-time 5 "$VLLM_HEALTH_URL" >/dev/null 2>&1; then
    echo "vLLM API ready:   $VLLM_API_BASE"
  else
    echo "vLLM API offline: Qwen3-14B BF16 will work after start-vllm is run on the server."
  fi
